<template>
  <div class="content-wrapper">
    ...
    <br>
    <br>
    <button class="my-button" v-on:click="getPrivateFolder('inner',function(v) {
      ElMessage.success(`getPrivateFolder JS收到SDK响应：${v}`)
    })">获取私有目录</button>
    <br>
    <br>
...
export default {
  ...
  methods: {
    ...
    // 窗口初始化时注册拓展能力
    registerEC(modules){
      window.dsBridge.call("async",`{"event":"ECEvent","function":"expandingCapacityRegister","params":{"providerModules":[${modules}]}}`,function(v) {
        ElMessage.success(`registerEC JS收到SDK响应：${v}`)
      })
    },
    initDataChannel() {
      // 监听数据通道事件
    ...
    getBufferedAmount() {
      // 查询bufferedAmount
      this.dataChannel.getBufferedAmount(function(v) {
        ElMessage.success(`getBufferedAmount JS收到SDK响应：${v}`)
      })
    },
    getLocation() {
      // 获取当前位置
    ...
    getFileList() {
      ...
      window.dsBridge.call(
          "async",
          `{"event":"FileEvent","function":"getFileList","params":{"folderPath":"/sdcard","offset":"0","count":"20","fileType":"0","sortType":"sizeSort","sortOrder":"1","folderPosition":"1","suffix":""}}`,function(v) {
            ElMessage.success(`getFileList JS收到SDK响应：${v}`)
          })
    },
    getPrivateFolder(type,f) {
      // 删除文件
      window.dsBridge.call("async", `{"event":"FileEvent","function":"getPrivateFolder","params":{"type":"${type}"}}`,f)
    },
    startSaveFile(path) {
      // 开始保存文件
      window.dsBridge.call("async", `{"event":"FileEvent","function":"startSaveFile","params":{"path":"${path}","append":true}}`,function(v) {
        ElMessage.success(`startSaveFile JS收到SDK响应：${v}`)
      })
    },
    ...
    stopSaveFile() {
      // 结束保存文件
      window.dsBridge.call("async", `{"event":"FileEvent","function":"stopSaveFile","params":{}}`,function(v) {
        ElMessage.success(`stopSaveFile JS收到SDK响应：${v}`)
      })
    },
    deleteFile(path) {
      // 删除文件
      window.dsBridge.call("async", `{"event":"FileEvent","function":"deleteFile","params":{"path":"${path}"}}`,function(v) {
        ElMessage.success(`deleteFile JS收到SDK响应：${v}`)
      })
    },
    startReadFile(path) {
      // 开始读取文件
      window.dsBridge.call("async", `{"event":"FileEvent","function":"startReadFile","params":{"path":"${path}"}}`,function(v) {
        ElMessage.success(`startReadFile JS收到SDK响应：${v}`)
      })
    },
    readFile(length,f) {
      // 读取文件
    ...
    stopReadFile() {
      // 结束读取文件
      window.dsBridge.call("async", `{"event":"FileEvent","function":"stopReadFile","params":{}}`,function(v) {
        ElMessage.success(`stopReadFile JS收到SDK响应：${v}`)
      })
    },
    ...
    checkFileOrFolderExists() {
      // 判断文件/文件夹是否存在
      window.dsBridge.call("async", `{"event":"FileEvent","function":"checkFileOrFolderExists","params":{"path":["/sdcard/haha","/sdcard/7.png"]}}`,function(v) {
        ElMessage.success(`checkFileOrFolderExists JS收到SDK响应：${v}`)
      })
    },
    saveUpdateKeyValueWithExpiry(key,value,ttl){
      // 缓存数据带有效时长
      window.dsBridge.call("async", `{"event":"FileEvent","function":"saveUpdateKeyValueWithExpiry","params":{"key":"${key}","value":"${value}","ttl":"${ttl}"}}`,function(v) {
        ElMessage.success(`saveUpdateKeyValueWithExpiry JS收到SDK响应：${v}`)
      })
    },
    getKeyValue(key){
      // 缓存数据带有效时长
      window.dsBridge.call("async", `{"event":"FileEvent","function":"getKeyValue","params":{"key":"${key}"}}`,function(v) {
        ElMessage.success(`getKeyValue JS收到SDK响应：${v}`)
      })
    },
    ...
    answer() {
      // 接听电话
      window.dsBridge.call("async", `{"event":"MiniAppEvent","function":"answer","params":{}}`,function(v) {
        ElMessage.success(`answer JS收到SDK响应：${v}`)
      });
    },
    hangUp() {
      // 挂断电话
      window.dsBridge.call("async", `{"event":"MiniAppEvent","function":"hangUp","params":{}}`,function(v) {
        ElMessage.success(`hangUp JS收到SDK响应：${v}`)
      });
    },
    getCallState() {
      // 获取通话状态
      window.dsBridge.call("async", `{"event":"MiniAppEvent","function":"getCallState","params":{}}`,function(v) {
        ElMessage.success(`getCallState JS收到SDK响应：${v}`)
      });
    },
    getRemoteNumber() {
      // 获取对端号码
    ...
    requestStartAdverseApp() {
      // 拉起对端小程序
      window.dsBridge.call("async", `{"event":"MiniAppEvent","function":"requestStartAdverseApp","params":{}}`,function(v) {
        ElMessage.success(`requestStartAdverseApp JS收到SDK响应：${v}`)
      });
    },
    setSystemApiLicense(l) {
      // 设置系统api license
      window.dsBridge.call("async", `{"event":"MiniAppEvent","function":"setSystemApiLicense","params":{"license":"${l}","api":"getUserMedia"}}`,function(v) {
        ElMessage.success(`setSystemApiLicense JS收到SDK响应：${v}`)
      });
    },
    getMiniAppInfo() {
      const that = this;
    ...
    stopApp() {
      // 关闭小程序
      window.dsBridge.call("async",`{"event":"MiniAppEvent","function":"stopApp","params":{}}`,function(v) {
        ElMessage.success(`stopApp JS收到SDK响应：${v}`)
      });
    },
    getHttpResult() {
      // 打开app组件
    ...
    addContact() {
      window.dsBridge.call("async", `{"event":"MiniAppEvent","function":"addContact","params":{"contactMode":"0", "number":"19129952167"}}`,function(v) {
        ElMessage.success(`addContact JS收到SDK响应：${v}`)
      });

    },
    editContact() {
      window.dsBridge.call("async", `{"event":"MiniAppEvent","function":"addContact","params":{"contactMode":"1", "number":"19129952157"}}`,function(v) {
        ElMessage.success(`editContact JS收到SDK响应：${v}`)
      });
    },
    getContactName() {
      // 需配置MINIAPP_READ_CONTACTS权限
      window.dsBridge.call("async", `{"event":"MiniAppEvent","function":"getContactName","params":{"contactNumber":"19120050940"}}`,function(v) {
        ElMessage.success(`getContactName JS收到SDK响应：${v}`)
      });
    },
    getContactList() {
      // 需配置MINIAPP_READ_CONTACTS权限
    ...
    stopScreenShare(){
      // 停止屏幕共享
      window.dsBridge.call("async", `{"event":"ScreenShareEvent","function":"stopScreenShare","params":{}}`,function(v) {
        ElMessage.success(`stopScreenShare JS收到SDK响应：${v}`)
      });
    },
    openSketchBoard(){
      // 开启画板
      window.dsBridge.call("async", `{"event":"ScreenShareEvent","function":"openSketchBoard","params":{}}`,function(v) {
        ElMessage.success(`openSketchBoard JS收到SDK响应：${v}`)
      });
    },
    closeSketchBoard(){
      // 关闭画板
      window.dsBridge.call("async", `{"event":"ScreenShareEvent","function":"closeSketchBoard","params":{}}`,function(v) {
        ElMessage.success(`closeSketchBoard JS收到SDK响应：${v}`)
      });
    },
    addDrawingInfo(data,role){
      // 绘制数据到画板
      window.dsBridge.call("async", `{"event":"ScreenShareEvent","function":"addDrawingInfo","params":{"drawingInfo":"${data}","role":${role}}}`,function(v) {
        ElMessage.success(`addDrawingInfo JS收到SDK响应：${v}`)
      });
    },
    requestECAIDetect() {
      // 调用拓展能力--AI视频检测
      window.dsBridge.call("async",`{"event":"ECEvent","function":"expandingCapacityRequest","params":{"provider":"OEM","module":"AIVideo","func":"detect","data":{}}}`,function(v) {
        ElMessage.success(`requestECAIDetect JS收到SDK响应：${v}`)
      });
    },
    requestECStartTranslate() {
      // 调用拓展能力--开始实时翻译
      window.dsBridge.call("async",`{"event":"ECEvent","function":"expandingCapacityRequest","params":{"provider":"OEM","module":"Translate","func":"start","data":{"meTo":"English","otherTo":"Chinese"}}}`,function(v) {
        ElMessage.success(`requestECStartTranslate JS收到SDK响应：${v}`)
      });
    },
    requestECStopTranslate() {
      // 调用拓展能力--停止实时翻译
      window.dsBridge.call("async",`{"event":"ECEvent","function":"expandingCapacityRequest","params":{"provider":"OEM","module":"Translate","func":"stop","data":{}}}`,function(v) {
        ElMessage.success(`requestECStopTranslate JS收到SDK响应：${v}`)
      });
    },
    requestECTranslateLanguageList() {
      // 调用拓展能力--获取实时翻译可选语种
      window.dsBridge.call("async",`{"event":"ECEvent","function":"expandingCapacityRequest","params":{"provider":"OEM","module":"Translate","func":"languageList","data":{}}}`,function(v) {
        ElMessage.success(`requestECTranslateLanguageList JS收到SDK响应：${v}`)
      });
    },
    requestECTranslateVoice() {
      // 调用拓展能力--设置实时翻译音频通道声音类型
      window.dsBridge.call("async",`{"event":"ECEvent","function":"expandingCapacityRequest","params":{"provider":"OEM","module":"Translate","func":"voice","data":{"type":"${this.voiceType}"}}}`,function(v) {
        ElMessage.success(`requestECTranslateVoice JS收到SDK响应：${v}`)
    ...
    selectAndCopyFile() {
      ...
      this.selectFile(function(v) {
        ...
        that.getPrivateFolder('outer',function (getPrivateFolderResponseJson) {
    ...
    playVoice(path) {
      window.dsBridge.call("async", `{"event":"FileEvent","function":"playVoice","params":{"path":"${path}"}}`,function(v) {
        ElMessage.success(`playVoice JS收到SDK响应：${v}`)
      });
    },
    stopPlayVoice() {
     window.dsBridge.call("async", `{"event":"FileEvent","function":"stopPlayVoice","params":{}}`,function(v) {
        ElMessage.success(`stopPlayVoice JS收到SDK响应：${v}`)
      });
    },
    openGaoDeMap(){
      // 高德坐标（GCJ20）
    ...
    setSpeakerphone(param){
      window.dsBridge.call("async", `{"event":"MiniAppEvent","function":"setSpeakerphone","params":{"speakerphoneOn":${param}}}`,function(v) {
        ElMessage.success(`setSpeakerphone JS收到SDK响应：${v}`)
      });
    },
    isSpeakerphoneOn(){
    ...
    setMuted(param){
      window.dsBridge.call("async", `{"event":"MiniAppEvent","function":"setMuted","params":{"muted":${param}}}`,function(v) {
        ElMessage.success(`setMuted JS收到SDK响应：${v}`)
      });
    },
    isMuted(){
    ...
    playDtmfTone(param){
      ...
      window.dsBridge.call("async", `{"event":"MiniAppEvent","function":"playDtmfTone","params":{"digit":"${param}","license":"${license}"}}`,function(v) {
        ElMessage.success(`playDtmfTone JS收到SDK响应：${v}`)
      });
    },
    ...
    addHtmlView(id,showOutCallPage,left,top,width,height){
      ...
      const license = "a+nKBsjO3nhpIOlh95cmc5Vx0VxIpUrHAVN1Fs8wXUMBxsUtpDdxHDrgDTHXTS3V34Ekkk+AcRIfWqWsJC77RSzuKHgWHrEIQczAr7B0Iw/vBzx9uiW09+5o3r+evp4RN"
      window.dsBridge.call("async",`{"event":"ECEvent","function":"expandingCapacityRequest","params":{"provider":"CT","module":"FloatView","func":"addHtmlView","data":{"license":"${license}","viewId":"${id}","htmlBase64":"${bs}","showOutCallPage":${showOutCallPage},"left":"${left}","top":"${top}","width":"${width}","height":"${height}","noToolbar":false}}}`,function(v) {
        console.info(`addHtmlView JS收到SDK响应：${v}`)
        // 每秒更新更新一次浮窗内容
    ...
    updateHtmlView(id,content,height){
      ...
      window.dsBridge.call("async",`{"event":"ECEvent","function":"expandingCapacityRequest","params":{"provider":"CT","module":"FloatView","func":"updateHtmlView","data":{"viewId":"${id}","htmlBase64":"${bs}"}}}`,function(v) {
        console.info(`updateHtmlView JS收到SDK响应：${v}`)
      });
    },
    ...
    addIconView(id,left,top,width,height){
      ...
      window.dsBridge.call("async",`{"event":"ECEvent","function":"expandingCapacityRequest","params":{"provider":"CT","module":"FloatView","func":"addIconView","data":{"license":"${license}","viewId":"${id}","iconBase64":"${bs}","left":"${left}","top":"${top}","width":"${width}","height":"${height}"}}}`,function(v) {
        console.info(`addIconView JS收到SDK响应：${v}`)
      });    },
    removeView(id){
      window.dsBridge.call("async",`{"event":"ECEvent","function":"expandingCapacityRequest","params":{"provider":"CT","module":"FloatView","func":"removeView","data":{"viewId":"${id}"}}}`,function(v) {
        console.info(`removeView JS收到SDK响应：${v}`)
      });    },
